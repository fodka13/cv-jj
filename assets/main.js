...
// });
  ...
  function injectAuthModal() {
    ...
    <div class="modal fade" id="authModal" tabindex="-1" aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="background: #111; border: 1px solid #333; border-radius: 15px; color: #fff;">
          ...
          <div class="modal-body p-5 pt-0">
            ...
            <form id="modal-login-form">
                <h3 class="cs-section_title mb-4 text-center">تسجيل الدخول</h3>
                <input type="email" class="cs-form_field mb-3" placeholder="البريد الإلكتروني" required id="modal-login-email">
                <input type="password" class="cs-form_field mb-3" placeholder="كلمة المرور" required id="modal-login-password">
                <button type="submit" class="cs-btn cs-style1 w-100 mb-3">دخول</button>
                
                <div class="d-flex align-items-center mb-3">
                    <hr class="flex-grow-1 border-secondary"><span class="px-3 small">أو</span><hr class="flex-grow-1 border-secondary">
                </div>
                
                <a href="/api/auth/google" class="cs-btn cs-style1 cs-type1 w-100 mb-4 py-2" style="font-size: 0.9rem;">
                    <img src="https://www.gstatic.com/firebasejs/ui/2.0.0/images/auth/google.svg" width="18" class="ms-2"> الربط المباشر عبر Google
                </a>

                <p class="mt-3 text-center small">ليس لديك حساب؟ <span class="cs-accent_color" style="cursor:pointer; text-decoration:underline" onclick="switchAuth('signup')">إنشاء حساب</span></p>
                <p class="text-center small"><span class="cs-accent_color" style="cursor:pointer; text-decoration:underline" onclick="switchAuth('forgot')">نسيت كلمة المرور؟</span></p>
            </form>

            <!-- Sign Up Form -->
            <form id="modal-signup-form" class="d-none">
                <h3 class="cs-section_title mb-4 text-center">إنشاء حساب جديد</h3>
                <input type="text" class="cs-form_field mb-3" placeholder="اسم المستخدم" required id="modal-signup-user">
                <input type="email" class="cs-form_field mb-3" placeholder="البريد الإلكتروني" required id="modal-signup-email">
                <input type="password" class="cs-form_field mb-3" placeholder="كلمة المرور" required id="modal-signup-password">
                <button type="submit" class="cs-btn cs-style1 w-100">تسجيل</button>
                <p class="mt-3 text-center small">لديك حساب بالفعل؟ <span class="cs-accent_color" style="cursor:pointer; text-decoration:underline" onclick="switchAuth('login')">تسجيل الدخول</span></p>
            </form>

            <!-- Forgot Password -->
            <div id="modal-forgot-form" class="d-none">
                <h3 class="cs-section_title mb-4 text-center">استعادة الحساب</h3>
                <div id="modal-step-1">
                    <input type="email" class="cs-form_field mb-3" placeholder="بريدك الإلكتروني" id="modal-reset-email">
                    <button type="button" class="cs-btn cs-style1 w-100" onclick="sendResetCode()">إرسال الرمز</button>
                </div>
                <div id="modal-step-2" class="d-none">
                    <input type="text" class="cs-form_field mb-3" placeholder="رمز التحقق (6 أرقام)" id="modal-reset-code">
                    <input type="password" class="cs-form_field mb-3" placeholder="كلمة المرور الجديدة" id="modal-reset-new-password">
                    <button type="button" class="cs-btn cs-style1 w-100" onclick="handleResetPassword()">تحديث</button>
                </div>
                <p class="mt-3 text-center small"><span class="cs-accent_color" style="cursor:pointer; text-decoration:underline" onclick="switchAuth('login')">العودة للدخول</span></p>
            </div>
          </div>
        </div>
      </div>
  ...
  window.switchAuth = function(type) {
    $('#modal-login-form, #modal-signup-form, #modal-forgot-form').addClass('d-none');
    $(`#modal-${type}-form`).removeClass('d-none');
  };

  async function checkAuthState() {
    const res = await fetch('/check-session');
    const data = await res.json();
    const navList = $('.cs-nav_list');
    
    // تنظيف العناصر المضافة سابقاً لمنع التكرار
    $('#logout-item, #admin-link, #login-item').remove();

    if (data.user) {
        const dashboardBtn = data.user.role === 'ADMIN' ? 
            `<li id="admin-link"><a href="admin-dashboard.html" class="cs-accent_color">لوحة التحكم</a></li>` : '';
        
        navList.append(`
            ${dashboardBtn}
            <li id="logout-item"><a href="#" id="logout-btn">خروج (${data.user.username})</a></li>
        `);
  ...
  // Modal Handlers
  $(document).on('submit', '#modal-login-form', async (e) => {
    e.preventDefault();
    const res = await fetch('/login', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({email: $('#modal-login-email').val(), password: $('#modal-login-password').val()})
    });
    if(res.ok) window.location.reload();
    else alert('خطأ في البريد أو كلمة المرور');
  });

  $(document).on('submit', '#modal-signup-form', async (e) => {
    e.preventDefault();
    const res = await fetch('/register', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({
            username: $('#modal-signup-user').val(),
            email: $('#modal-signup-email').val(),
            password: $('#modal-signup-password').val()
        })
    });
    if(res.ok) { alert('تم التسجيل بنجاح'); switchAuth('login'); }
  });
  ...
  window.sendResetCode = async () => {
    await fetch('/forgot-password', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({email: $('#modal-reset-email').val()})
    });
    $('#modal-step-1').addClass('d-none');
    $('#modal-step-2').removeClass('d-none');
  };
